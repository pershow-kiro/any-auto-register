...
def sync_account(account) -> list[dict[str, Any]]:
    ...
    def _build_chatgpt_upload_account():
        ...
        a.account_id = extra.get("account_id") or getattr(account, "user_id", "") or ""
        a.user_id = getattr(account, "user_id", "") or ""
        a.extra = extra
        return a
    ...
    if platform == "chatgpt":
        ...
        sub2api_url = str(
            config_store.get("sub2api_api_url", "")
            or config_store.get("sub2api_url", "")
            or ""
        ).strip()
